...
# 4. UPDATE (PUT) - Повне оновлення даних
...
def get_students_by_group(group_name: str, db: Session = Depends(get_db)):
    return db.query(models.Student).filter(models.Student.group == group_name).all()
...
def get_students_above_grade(min_grade: float, db: Session = Depends(get_db)):
    return db.query(models.Student).filter(models.Student.grade >= min_grade).all()

# Завдання 7.1: Пошук за іменем (частковий збіг)
...
# Завдання 7.2: PATCH - Часткове оновлення
@app.patch("/students/{student_id}", response_model=StudentResponse)
def partial_update_student(student_id: int, student_update: StudentUpdate, db: Session = Depends(get_db)):
    student = db.query(models.Student).filter(models.Student.id == student_id).first()
    if not student:
        raise HTTPException(status_code=404, detail="Студента не знайдено")
    
    if student_update.name is not None: student.name = student_update.name
    if student_update.group is not None: student.group = student_update.group
    if student_update.grade is not None: student.grade = student_update.grade
    if student_update.email is not None: student.email = student_update.email
    
    db.commit()
    db.refresh(student)
    return student

# Завдання 7.3: Статистика по групах
...
# Завдання 8: Масове оновлення оцінок (Транзакції)
@app.post("/students/bulk-update-grades")
def bulk_update_grades(updates: List[GradeUpdate], db: Session = Depends(get_db)):
    try:
        count = 0
        for update in updates:
            student = db.query(models.Student).filter(models.Student.id == update.student_id).first()
            if student:
                student.grade = update.new_grade
                count += 1
        
        db.commit() 
        return {"message": f"Успішно оновлено {count} оцінок"}
    
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Помилка транзакції: {str(e)}")